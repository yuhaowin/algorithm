...
/**
 * leet code question 45 followup
 */
public class Q45JumpGameIIFollowUp {
    ...
    // 返回最后到达aim位置，跳的最少的步数
    // 如果返回-1表示怎么也无法到达
    public static int h(int N, int aim, int[] arr, int i, int k, int[][] dp) {
        if (i < 1 || i > N || k > N - 1) {
            return -1;
        }
        if (dp[i][k] != -2) {
        ...
        int ans1 = h(N, aim, arr, i + arr[i - 1], k + 1, dp);
        int ans2 = h(N, aim, arr, i - arr[i - 1], k + 1, dp);
        int ans = -1;
        if (ans1 != -1 && ans2 != -1) {
            ans = Math.min(ans1, ans2);
        }
    ...
    // bfs
    public static int jump4bfs(int N, int start, int end, int[] arr) {
