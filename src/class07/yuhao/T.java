 ...
 * arr =[3,3,1,2,1,2,5...]
 * op  =[T,T,T,T,F,T,F...]
 * 依次表示：3号用户购买了一件商品，3号用户购买了一件商品，1号用户购买了一件商品，
 * 2号用户购买了一件商品，1号用户退货了一件商品，2号用户购买了一件商品，5号用户退货了一件商品...
 * <p>
 * 一对 arr[i] 和 op[i] 就代表一个事件：
 * 用户号为arr[i]，op[i]==T 就代表这个用户购买了一件商品,op[i]==F 就代表这个用户退货了一件商品
 ...
 * 进入候选区的时间就是当前事件的时间（可以理解为ar[和opD中的i）
 * 从候选区出来进入得奖区的用户，候选区时间删除，
 * 进入得奖区的时间就是当前事件的时间（可以理解为arr和op的中的）
